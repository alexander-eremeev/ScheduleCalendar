 ...
 */
class HomePageViewModel(itemsRepository: ItemsRepository) : ViewModel() {
    ...
    val homePageUiState: StateFlow<HomePageUiState> =
        itemsRepository.getAllItemsStream().map { HomePageUiState(it) }
            .stateIn(
 ...
 */
data class HomePageUiState(val itemList: List<Item> = listOf())
