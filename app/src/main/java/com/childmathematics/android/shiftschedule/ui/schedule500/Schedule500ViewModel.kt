 ...
 */
class Schedule500ViewModel(itemsRepository: ItemsRepository) : ViewModel() {
     ...
     */
    val schedule500UiState: StateFlow<Schedule500UiState> =
        itemsRepository.getAllItemsStream().map { Schedule500UiState(it) }
            .stateIn(
                scope = viewModelScope,
                started = SharingStarted.WhileSubscribed(TIMEOUT_MILLIS),
                initialValue = Schedule500UiState()
            )
 ...
 */
data class Schedule500UiState(val itemList: List<Item> = listOf())
