 ...
 */
class AboutViewModel(itemsRepository: ItemsRepository) : ViewModel() {
    ...
    val aboutUiState: StateFlow<AboutUiState> =
        itemsRepository.getAllItemsStream().map { AboutUiState(it) }
            .stateIn(
 ...
 */
data class AboutUiState(val itemList: List<Item> = listOf())
