    ...
    Scaffold(
        ...
        ItemEntryBody(
            ...
            onSaveClick = {
                ...
                // Примечание. Если пользователь поворачивает экран очень быстро, операция может быть отменена
                // и элемент не может быть обновлен в базе данных. Это потому, что при настройке
                // происходит изменение, Activity будет воссоздано, а RememberCoroutineScope будет
                // быть отменено, поскольку область действия привязана к композиции.
...
@Composable
fun ItemEditScreenPreview() {//ScheduleCalendar
